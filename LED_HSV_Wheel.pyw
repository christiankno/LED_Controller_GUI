...
class ColorScale(tk.Canvas):
    ...
    def _draw_gradient(self, val):
        ...
        if self.color_grad == 'bw':
            ...
            def f(i):
                line.append(tuple2hex(colorsys.hsv_to_rgb((float(i) / width * 360),1,255)))
        else:
...
h=ColorScale(root, to=65535., command=setH)
s=ColorScale(root, from_=0, to=65535, command=setS, gradient='wb')
v=ColorScale(root, from_=0, to=65535, command=setV, gradient='bw')
h.set(hsv[0])
s.set(hsv[1])
v.set(hsv[2])
